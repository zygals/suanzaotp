...
{block name="title"}{$modelName}列表{/block}
{block name="content"}
<style>
...
<div role="tabpanel" class="tab-pane" id="user" style="display:block;">
    <div class="check-div form-inline row">
        <div class="col-xs-2">
            <a href="{:url('create')}">
                <button class="btn btn-yellow btn-xs">添加{$modelName}</button>
            </a>
        </div>
        <div class="col-xs-10">
            ...
            <?php if (count($list_) > 0) { ?>
                <?php foreach ($list_ as $key => $row_) { ?>
                    <div class="row cont_nowrap">
                        ...
                        <div class="col-xs-2" title="__IMGURL__{$row_->path}">
                            __IMGURL__{$row_->path}
                        </div>
